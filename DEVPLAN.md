...
**Done:** `tests/test_faceoffs.py` — win probability monotonic in center rating gap, icing/offside faceoffs correctly logged with the right stoppage-type context. `tests/test_injuries.py` — injury rates within a sane band over a season; injured players excluded from lineup/rotation selection until healed; full `tests/test_engine.py`/`test_season.py` still pass.
