...
class Player:
    ...
    # Which feeder path this player came up through -- DESIGN.md point 11's
    ...
    # See config.LEAGUE_ORIGIN_CHOICES for the legal values.
    #
    # This field sat at the inert "none" default from Step 1.6 until the prospect
    # development round (docs/PROSPECT_DEV_PLAN.md) gave it consumers: it is now
    # populated at generation from the prospect's pre-draft level, and read by
    # systems/prospects.py to decide which development tiers a player is eligible
    # for. Two real rules hang off it -- a major-junior player is permanently NCAA-
    # ineligible, and the CHL-NHL transfer agreement bars a drafted junior player
    # under 20 from the AHL. Established (non-prospect) players generated by
    # leaguegen still carry "none"; nothing reads origin for a player who is
    ...
    # Where this player currently is in the development system, or None for anyone
    # not in it (every established NHL player, and every ordinary free agent).
    # Owned SOLELY by systems/prospects.py -- nothing else should construct or
    # mutate this dict; read it through that module's accessors.
    #
    #   {"tier": str,               # config.DEV_TIERS -- chl / ncaa / ahl / europe
    #    "seasons": int,            # total seasons developing, across all tiers
    #    "tier_seasons": int,       # seasons in the CURRENT tier (drives NCAA's
    #                               #   four-year eligibility clock)
    #    "rights_tid": int | None,  # team holding this player's rights; None means
    #                               #   nobody does -- the undrafted/UDFA track
    #    "rights_expire": int|None, # season year those rights lapse
    #    "line": dict}              # synthetic season stat line, flavor only
    #
    # A plain dict rather than a dataclass, following the same reasoning the
    # already-shipped ``draft``/``pre_draft`` bios and ``World.bracket``/``history``
    # use: it is JSON-native, so it round-trips through to_dict/from_dict with zero
    # extra serialization code, and exactly one module owns its internal shape.
    #
    # Note this does NOT make the player a member of ``rights_tid``'s team --
    # ``team_id`` stays None for a prospect, since a prospect is not on the NHL
    # active roster. That is what keeps a signed prospect's entry-level contract off
    # the team's cap (``cap.payroll`` sums over ``Team.roster``), matching the real
    # rule that junior and minor-league contracts don't count against the NHL cap.
    ...
    def is_prospect(self) -> bool:
        """Is this player currently developing in a feeder tier (see ``development``)?

        Note a prospect is ALSO ``is_free_agent`` -- he holds no NHL roster spot, and
        deliberately stays in ``World.free_agents`` so no separate pool has to be kept
        in sync. Callers asking "may this player be signed right now?" want
        ``systems.prospects.is_reserved_prospect``, which is the rules question; this
        property is only the data question of whether a development record exists.
        """
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "development": dict(self.development) if self.development else None,
    ...
    def from_dict(cls, d: dict) -> "Player":
        ...
        return cls(
            ...
            development=(dict(d["development"]) if d.get("development") else None),
